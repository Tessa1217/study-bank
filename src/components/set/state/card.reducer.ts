...
export function cardReducer(state: CardState, action : CardAction) : CardState {
  switch (action.type) {
    case "INIT":      
      return {...state, cards: action.payload.sort((a, b) => a.sort_order - b.sort_order), activeId : action.payload[0]?.id}
    case "ADD_CARD": 
    ...
    case "UPDATE_LANG" : {
      const cards = state.cards.map(c => c.id === action.id ? {...c, [`${action.side}_lang`] : action.lang } : c)
      return { ...state, cards};
    }
    case "REMOVE_CARD": {
      const cards = state.cards.filter(c => c.id !== action.id)
        .map((c, i) => ({ ...c, order: i + 1 }));
      const activeId = state.activeId === action.id ? cards[0]?.id : state.activeId;
      return { ...state, cards, activeId };
    }
    case "REORDER": {
      const sorted_cards = [...action.payload];
      sorted_cards.forEach((sorted_card, new_idx) => sorted_card.sort_order = new_idx + 1)
