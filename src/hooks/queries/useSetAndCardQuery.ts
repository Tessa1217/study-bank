...
export function useSaveSetWithCardsMutation() {
  ...
  return useMutation({
    mutationFn: async (payload: { set: StudySetDraft; cards: StudyCardDraft[] }) => {
      const dto = { set: toSetDTO(payload.set), cards: toCardsDTO(payload.cards) };
      return callEdge<{ data: { set: any; cards: any[] } }>('save-set-and-cards', dto);
    },
    onSuccess: (result) => {
